  ...
  public partial class MainForm : DevExpress.XtraBars.Ribbon.RibbonForm {
    ...
    private void tabbedView_QueryControl(object sender, QueryControlEventArgs e) {
      e.Control = e.Document.ControlName switch
      {
        ...
        "PrintPreview" => new PrintPreview(e.Document.Tag as Action<PrintPreview>),
        _ => new Control()
      };
    }
    ...
    void ShowPrintPreview(string caption, Action<PrintPreview> initPreview) {
      ShowOrActivate<PrintPreview>($"{caption} (Print Preview)")(document => {
        document.Tag = initPreview;
      });
    }

    public void ShowPrintPreview(string caption, PrintingSystemBase printingSystem) {
      ShowPrintPreview(caption, pp => pp.ShowDocument(printingSystem));
    }
